...
namespace RHCQS_BE.Controllers
{
    ...
    public class FinalQuotationController : ControllerBase
    {
        ...
        public FinalQuotationController(IFinalQuotationService finalQuotationService, IFirebaseService firebaseService, IAccountService accountService, IGmailSenderService gmailSenderService)
        {
        ...
        public async Task<IActionResult> ApproveFinalFromManager([FromQuery] Guid finalId, [FromBody] ApproveQuotationRequest request)
        {
            ...
            if (!string.IsNullOrEmpty(pdfUrl))
            {
                ...
                var notificationRequest = new NotificationRequest
                {
                    ...
                    Body = @$"Báo giá chi tiết của dự án có mã là {customerEmail.ProjectCode} 
đã có cập nhật mới bạn cần xem."
        ...
        Cảm ơn, RHCQS team";
                ...
                var sendemail = new EmailRequest
                {
                    ...
                    Subject = "Cập nhật mới về báo giá chi tiết",
                    Body = emailBody
                };

                // Gộp chung cả hai tác vụ vào một Task.Run
                Task.Run(async () =>
                {
                    await Task.WhenAll(
